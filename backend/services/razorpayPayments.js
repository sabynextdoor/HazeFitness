...
async function finalizePayment({ orderId, paymentId, mock = false }) {
  ...
  return db.transaction(async (tx) => {
    ...
    const mode = mock ? 'other' : paymentMode(gatewayPayment.method);
    const result = await tx.run(`
      INSERT INTO payments (subscription_id, member_id, amount, payment_mode, payment_date, receipt_no, note)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [order.subscription_id, order.member_id, order.amount, mode,
